...
namespace Base45Test
{
    public class Tests
    {
        ...
        [Test]
        public void EmptyEncodeDecode_BytesAndString()
        {
            var base45 = new Base45();
            Assert.AreEqual(string.Empty, base45.Encode(new byte[0]));
            Assert.AreEqual(string.Empty, base45.Encode(string.Empty));
            CollectionAssert.IsEmpty(base45.Decode(string.Empty));
            Assert.AreEqual(string.Empty, base45.DecodeAsString(string.Empty));
        }
        ...
        {
            var base45 = new Base45();
            Assert.Throws<ArgumentNullException>(() => base45.Encode((byte[])null));
            Assert.Throws<ArgumentNullException>(() => base45.Encode((string)null));
            Assert.Throws<ArgumentNullException>(() => base45.Decode((string)null));
        }

        [Test]
        public void SingleByte_Roundtrip_ZeroAndMax()
        {
            var base45 = new Base45();

            var b0 = new byte[] { 0 };
            var enc0 = base45.Encode(b0);
            var dec0 = base45.Decode(enc0);
            Assert.IsTrue(dec0.SequenceEqual(b0));
            Assert.AreEqual(base45.DecodeAsString(enc0), Encoding.UTF8.GetString(dec0));

            var b255 = new byte[] { 255 };
            var enc255 = base45.Encode(b255);
            var dec255 = base45.Decode(enc255);
            Assert.IsTrue(dec255.SequenceEqual(b255));
        }

        [Test]
        public void TwoAndThreeBytes_Roundtrip()
        {
            var base45 = new Base45();

            var two = new byte[] { (byte)'A', (byte)'B' };
            var encTwo = base45.Encode(two);
            Assert.IsTrue(base45.Decode(encTwo).SequenceEqual(two));

            var three = new byte[] { 0x01, 0x02, 0xFF };
            var encThree = base45.Encode(three);
            Assert.IsTrue(base45.Decode(encThree).SequenceEqual(three));
        }

        [Test]
        public void Utf8MultibyteString_Roundtrip()
        {
            var base45 = new Base45();
            var text = "€😀漢字"; // mix of multibyte code points
            var bytes = Encoding.UTF8.GetBytes(text);

            var encFromText = base45.Encode(text);
            var encFromBytes = base45.Encode(bytes);
            Assert.AreEqual(encFromText, encFromBytes);

            var decoded = base45.DecodeAsString(encFromText);
        ...
        public void AllByteValues_Roundtrip()
        {
            ...
            var encoded = base45.Encode(all);
            var decoded = base45.Decode(encoded);
            Assert.IsTrue(decoded.SequenceEqual(all));
        }

        [Test]
        public void DecodeWithInvalidCharacter_ThrowsInvalidOperationException()
        {
            var base45 = new Base45();
            // '!' is not part of Base45 alphabet; decoding must fail
